defmodule PhoenixKitCatalogue.LiveDatabaseGuardWiringTest do
  @moduledoc """
  S014: `PhoenixKitCatalogue.Test.LiveDatabaseGuardTest` calls `check!/1`
  ...
  for live_db <- ~w(phoenix_kit_dev decor_3d_print_dev phoenixkit_hello_world_dev) do
    test "refuses before any connection attempt when PGDATABASE=#{live_db}" do
      ...
      # loading makes every subprocess boot print "redefining module
      ...
      # a cut wiring call.
      assert output =~ "** (PhoenixKitCatalogue.Test.LiveDatabaseGuard.LiveDatabaseError)",
  ...
  test "an isolated test database name is not refused — the guard does not block a real run" do
    ...
    env = [{"PGDATABASE", System.get_env("PGDATABASE", "phoenix_kit_catalogue_test")}]
    ...
    refute output =~ "** (PhoenixKitCatalogue.Test.LiveDatabaseGuard.LiveDatabaseError)",
